...
type MsgItem = { type: 'message'; msg: TgMessage }
...
function sortByTime(msgs: TgMessage[]): TgMessage[] {
...
function displayName(conv: { chat_id: string; contact_name: string | null }) {
  return conv.contact_name || `Chat ${conv.chat_id}`
}

export default function TelegramMessagesView({ session }: TelegramMessagesViewProps) {
  ...
  const eligibleForReengage = useMemo(() => {
    return conversations.filter(c => !blockedIds.has(c.chat_id))
  ...
  const fetchConversations = useCallback(async () => {
    ...
    try {
      const { data, error } = await supabase
        .rpc('get_telegram_conversations', { p_user_id: session.user.id })

      if (error) {
        const { data: fallbackData } = await supabase
          .from('telegram_messages')
          .select('chat_id, contact_name, body, created_at, direction')
        ...
        if (fallbackData) {
          const grouped = new Map<string, TgConversation>()
          for (const msg of fallbackData) {
            if (!grouped.has(msg.chat_id)) {
              grouped.set(msg.chat_id, {
                chat_id: msg.chat_id,
  ...
  const toggleBlock = useCallback(async (chatId: string) => {
    setTogglingBlock(true)
    try {
      const isBlocked = blockedIds.has(chatId)
      if (isBlocked) {
        await supabase
          .from('blocked_contacts')
          .delete()
        ...
        await supabase
          .from('blocked_contacts')
          .insert({ user_id: session.user.id, contact_phone: chatId, channel: 'telegram', reason: 'Bloqueado desde bandeja' })
      }
      await fetchBlocked()
    } catch { /* silent */ }
  ...
  useEffect(() => {
    async function checkConnection() {
      const { data } = await supabase
        ...
        .eq('used', true)
        .limit(1)
      const hasPlatform = data != null && data.length > 0
      ...
      if (!hasPlatform) {
        const { data: inst } = await supabase
          ...
          .eq('user_id', session.user.id)
          .limit(1)
        const hasOwn = inst && inst.length > 0 && inst[0].channels?.telegram?.bot_type === 'own'
        setHasActiveConnection(hasOwn || false)
      } else {
  ...
  const fetchMessages = useCallback(async (chatId: string, fromOffset = 0) => {
    setLoadingMessages(true)
    try {
      const { data, error } = await supabase
        .from('telegram_messages')
        .select('id, chat_id, contact_name, direction, body, created_at, sender_type')
        .eq('user_id', session.user.id)
        .eq('chat_id', chatId)
        .order('created_at', { ascending: false })
        .order('id', { ascending: false })
      ...
      if (!error && data) {
        const sorted = sortByTime([...data].reverse() as TgMessage[])
        if (fromOffset === 0) {
  ...
  const handleNewMessage = useCallback((newMsg: TgMessage) => {
    lastMessageTimeRef.current = newMsg.created_at

    if (newMsg.chat_id === selectedContactRef.current) {
      setMessages(prev => {
        if (prev.some(m => m.id === newMsg.id)) return prev
        if (newMsg.direction === 'outbound') {
          let removedOne = false
          const cleaned = prev.filter(m => {
            if (!removedOne && m.id.startsWith('optimistic-') && m.body === newMsg.body && m.chat_id === newMsg.chat_id) {
              removedOne = true
              return false
            }
            return true
    ...
    setConversations(prev => {
      const exists = prev.some(c => c.chat_id === newMsg.chat_id)
      let updated = prev.map(c => {
        if (c.chat_id === newMsg.chat_id) {
          return {
            ...
            unread: newMsg.chat_id !== selectedContactRef.current && newMsg.direction === 'inbound'
              ? c.unread + 1 : c.unread
          }
        }
        return c
  ...
  useEffect(() => {
        ...
        try {
          ...
          if (data && data.length > 0) {
            for (const msg of data) {
              handleNewMessage(msg as TgMessage)
            }
          }
        } catch { /* silent */ }
      }, 5000)
    ...
    const channel = supabase
      .channel('tg-inbox-' + session.user.id)
      .on(
        'postgres_changes',
        {
          event: 'INSERT',
        ...
        (payload) => {
          handleNewMessage(payload.new as TgMessage)
        }
      )
      .subscribe((status) => {
  ...
  const handleSelectContact = useCallback(async (chatId: string) => {
    setSelectedContact(chatId)
    setHumanMode(humanModeContacts.has(chatId))
    ...
    try {
      ...
      if (!s) return
      const r = await fetch(`/api/telegram-link-start?action=check-mode&contact=${encodeURIComponent(chatId)}`, {
      ...
      setHumanModeContacts(prev => {
        ...
        if (isHuman) next.add(chatId); else next.delete(chatId)
        return next
      })
    } catch {}
  }, [humanModeContacts])
  ...
  const toggleHumanMode = useCallback(async () => {
    ...
    try {
      ...
      const r = await fetch(`/api/telegram-link-start?action=${action}`, {
        method: 'POST',
  ...
  const handleSend = useCallback(async () => {
    ...
    const selectedConv = conversations.find(c => c.chat_id === selectedContact)
    ...
    try {
      ...
      const res = await fetch('/api/telegram-link-start?action=send', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${currentSession.access_token}`
        },
        body: JSON.stringify({ chat_id: selectedContact, message: messageText })
      })
  ...
  const handleReengageSend = useCallback(async () => {
    ...
    const chatIds = Array.from(reengageSelected)
    setReengageProgress({ sent: 0, failed: 0, total: chatIds.length })
    ...
    try {
      ...
      const res = await fetch('/api/telegram-link-start?action=send', {
        ...
        body: JSON.stringify({ contacts: chatIds, message: reengageMessage.trim() })
      })
  ...
  const toggleReengageAll = useCallback(() => {
    if (reengageSelected.size === eligibleForReengage.length) {
      ...
      setReengageSelected(new Set(eligibleForReengage.map(c => c.chat_id)))
    }
  }, [reengageSelected.size, eligibleForReengage])

  const toggleReengageContact = useCallback((chatId: string) => {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-4 p-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Bandeja Telegram</h2>
          <p className="text-gray-500 text-sm">Gestiona las conversaciones de tu asistente IA en Telegram.</p>
        </div>
        <div className="flex items-center gap-2">
          <div className={`w-2 h-2 rounded-full ${
            realtimeStatus === 'connected' ? 'bg-[#0088cc]' :
            realtimeStatus === 'fallback' ? 'bg-yellow-500' :
            'bg-gray-500 animate-pulse'
          }`} />
          <span className={`text-[10px] uppercase tracking-wider ${
            realtimeStatus === 'connected' ? 'text-[#0088cc]' :
        ...
        <>
          {conversations.length === 0 ? (
            <div className="rounded-2xl bg-white border border-gray-200 shadow-sm p-12 text-center space-y-4">
              <div className="w-16 h-16 mx-auto rounded-2xl bg-[#0088cc]/10 flex items-center justify-center">
                <svg className="w-8 h-8 text-[#0088cc]/50" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M20 2H4c-1.1 0-2 .9-2 2v18l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm0 14H6l-2 2V4h16v12z"/>
                </svg>
              </div>
              <p className="text-gray-400 text-sm">Aun no hay conversaciones registradas.</p>
              <p className="text-gray-400 text-xs">Los mensajes apareceran aqui cuando tu bot comience a responder en Telegram.</p>
            </div>
          ) : (
              ...
              <div className="md:col-span-4 rounded-2xl bg-white border border-gray-200 shadow-sm overflow-hidden flex flex-col">
                <div className="p-3 border-b border-gray-100 space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <select
                      ...
                      className="bg-gray-50 border border-gray-200 rounded-lg px-2 py-1 text-[10px] text-gray-500 focus:outline-none focus:border-[#0088cc]/50 cursor-pointer"
                    >
                  ...
                  <input
                    ...
                    className="w-full bg-gray-50 border border-gray-200 rounded-lg px-3 py-1.5 text-xs text-gray-900 placeholder-gray-400 focus:outline-none focus:border-[#0088cc]/50"
                  />
                </div>
                <div className="flex-1 overflow-y-auto">
                  {filteredConversations.length === 0 ? (
                    <div className="p-6 text-center">
                      ...
                      <button
                        ...
                        className="text-[10px] text-[#0088cc] mt-2 hover:underline"
                      >
                        Ver todas
                      </button>
                    </div>
                  ...
                  {filteredConversations.map(conv => (
                    <button
                      key={conv.chat_id}
                      onClick={() => handleSelectContact(conv.chat_id)}
                      className={`w-full text-left p-4 border-b border-gray-100 transition-all hover:bg-gray-50 ${
                        selectedContact === conv.chat_id ? 'bg-[#0088cc]/10 border-l-2 border-l-[#0088cc]' : ''
                      }`}
                    >
                      <div className="flex items-center justify-between">
                        <span className={`text-sm font-medium ${blockedIds.has(conv.chat_id) ? 'text-red-400 line-through' : 'text-gray-900'}`}>
                          {displayName(conv)}
                        </span>
                        <div className="flex items-center gap-1.5">
                          {humanModeContacts.has(conv.chat_id) && (
                            <span className="text-[8px] bg-amber-50 text-amber-600 px-1.5 py-0.5 rounded-full font-bold">HUMANO</span>
                          )}
                          {blockedIds.has(conv.chat_id) && (
                            <span className="text-[8px] bg-red-50 text-red-500 px-1.5 py-0.5 rounded-full font-bold">BLOQ</span>
                          )}
                          {conv.unread > 0 && (
                            <span className="min-w-[18px] h-[18px] flex items-center justify-center rounded-full bg-[#0088cc] text-[10px] text-white font-bold px-1">
                              {conv.unread}
                            </span>
                          )}
                  ...
                  <>
                    <div className="p-4 border-b border-gray-100 space-y-2">
                      <div className="flex items-center gap-3">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center ${blockedIds.has(selectedContact) ? 'bg-red-50' : 'bg-[#0088cc]/20'}`}>
                          <span className={`text-xs font-bold ${blockedIds.has(selectedContact) ? 'text-red-500' : 'text-[#0088cc]'}`}>
                            {(conversations.find(c => c.chat_id === selectedContact)?.contact_name || selectedContact).slice(0, 2).toUpperCase()}
                          </span>
                        </div>
                        <div>
                          <p className="text-sm text-gray-900 font-medium">
                            {displayName(conversations.find(c => c.chat_id === selectedContact) || { chat_id: selectedContact, contact_name: null })}
                          </p>
                          <p className={`text-[10px] uppercase tracking-widest ${
                            blockedIds.has(selectedContact) ? 'text-red-500' :
                            hasActiveConnection ? 'text-[#0088cc]' : 'text-gray-400'
                          }`}>
                            ...
                            <div
                              className={`max-w-[75%] rounded-2xl px-4 py-2.5 ${
                                msg.direction === 'outbound'
                                  ? 'bg-[#0088cc]/10 border border-[#0088cc]/20 rounded-br-md'
                                  : 'bg-gray-50 border border-gray-200 rounded-bl-md'
                              } ${msg.id.startsWith('optimistic-') ? 'opacity-60' : ''}`}
                            >
                              <p className={`text-sm ${msg.direction === 'outbound' ? 'text-[#0088cc]' : 'text-gray-700'}`}>
                                {msg.body}
                              </p>
                              <div className={`flex items-center gap-1 mt-1 ${msg.direction === 'outbound' ? 'justify-end' : ''}`}>
                                <span className="text-[10px] text-gray-400">{formatTime(msg.created_at)}</span>
                                {msg.direction === 'outbound' && (
                                  <span className={`text-[10px] ${msg.sender_type === 'human' ? 'text-blue-500' : 'text-[#0088cc]/60'}`}>
                    ...
                    <div className="p-3 border-t border-gray-100">
                      {blockedIds.has(selectedContact) ? (
                        ...
                        <p className="text-[10px] text-gray-400 italic text-center">
                          Conecta tu Telegram para enviar mensajes.
                        </p>
                      ) : (
                        <div className="space-y-2">
                          ...
                          <div className="flex items-end gap-2">
                            <textarea
                              ...
                              disabled={sending}
                              rows={1}
                              className="flex-1 bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 text-sm text-gray-900 placeholder-gray-400 resize-none focus:outline-none focus:border-[#0088cc]/50 disabled:opacity-40"
                              style={{ maxHeight: '120px' }}
                            />
                            <button
                              onClick={handleSend}
                              disabled={sending || !newMessage.trim()}
                              className="p-2.5 rounded-xl bg-[#0088cc] hover:bg-[#006da3] disabled:opacity-30 disabled:cursor-not-allowed transition-all flex-shrink-0"
                            >
                              {sending ? (
      ...
      {internalTab === 'reengage' && (
        <div className="rounded-2xl bg-white border border-gray-200 shadow-sm overflow-hidden">
          <div className="p-4 border-b border-gray-100">
            <h3 className="text-sm font-bold text-gray-900">Mensaje masivo</h3>
            <p className="text-xs text-gray-400 mt-1">
              Envia un mensaje a todos tus contactos de Telegram.
              Los contactos bloqueados no apareceran en la lista.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-0 md:divide-x divide-gray-100">
            ...
            <div className="p-4 space-y-3">
              <div className="flex items-center justify-between">
                <p className="text-[10px] font-bold tracking-[0.18em] text-gray-400 uppercase">
                  Contactos ({eligibleForReengage.length})
                </p>
              ...
              {eligibleForReengage.length === 0 ? (
                <div className="text-center py-8">
                  <p className="text-xs text-gray-400">No hay contactos disponibles.</p>
                </div>
              ) : (
                  ...
                  {eligibleForReengage.map(conv => (
                    <label
                      ...
                      className={`flex items-center gap-3 p-3 rounded-xl cursor-pointer transition-all ${
                        reengageSelected.has(conv.chat_id) ? 'bg-[#0088cc]/5 border border-[#0088cc]/20' : 'hover:bg-gray-50 border border-transparent'
                      }`}
                    >
                      <input
                        type="checkbox"
                        checked={reengageSelected.has(conv.chat_id)}
                        onChange={() => toggleReengageContact(conv.chat_id)}
                        className="w-4 h-4 rounded border-gray-300 text-[#0088cc] focus:ring-[#0088cc]"
                      />
            ...
            <div className="p-4 space-y-3">
              ...
              <textarea
                ...
                rows={5}
                maxLength={4096}
                className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-900 placeholder-gray-400 resize-none focus:outline-none focus:border-[#0088cc]/50"
              />
              <div className="flex items-center justify-between">
                <span className="text-[10px] text-gray-400">{reengageMessage.length}/4096</span>
                <button
                  onClick={handleReengageSend}
                  disabled={reengageSending || reengageSelected.size === 0 || !reengageMessage.trim()}
                  className="px-4 py-2 rounded-xl bg-[#0088cc] hover:bg-[#006da3] text-white text-xs font-bold uppercase tracking-wider disabled:opacity-30 disabled:cursor-not-allowed transition-all flex items-center gap-2"
                >
                  {reengageSending ? (
                    <>
              ...
              {reengageResults && reengageResults.some(r => !r.ok) && (
                <div className="space-y-1 pt-2">
                  ...
                  {reengageResults.filter(r => !r.ok).map((r, i) => {
                    const conv = conversations.find(c => c.chat_id === r.chat_id)
                    return (
                      <p key={i} className="text-[10px] text-red-400">
                        {conv ? displayName(conv) : r.chat_id}: {r.error}
                      </p>
                    )
                  })}
                </div>
