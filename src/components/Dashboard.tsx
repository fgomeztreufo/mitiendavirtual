...
import FaqsView from './FaqsView' // <--- IMPORTANTE: Crea este archivo con el código anterior
    ...
    <div className="flex h-screen bg-black text-white font-sans overflow-hidden">
      ...
      <aside className="w-64 bg-gray-900 border-r border-gray-800 flex flex-col hidden md:flex">
        ...
        <div className="flex-1 p-4 space-y-2 overflow-y-auto">
            <p className="text-xs font-bold text-gray-500 uppercase px-2 mb-2">Canales</p>
      ...
      <main className="flex-1 overflow-y-auto relative bg-black flex flex-col justify-between">
        <div className="max-w-6xl mx-auto p-4 md:p-10 w-full flex-grow">
            ...
            {activeTab === 'catalog' && <CatalogView session={session} />}
            
            {activeTab === 'whatsapp' && <WhatsAppView />}
            
            {/* --- NUEVA VISTA PARA FAQs --- */}
...
const SidebarBtn = ({ icon, label, active, onClick, status }: any) => (
    <button 
        onClick={onClick} 
        className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-all ${active ? 'bg-blue-600/10 text-blue-400 border border-blue-600/20' : 'text-gray-400 hover:bg-gray-800'}`}
    >
        {icon}
