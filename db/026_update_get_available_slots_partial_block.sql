-- 026: Soporte para bloqueo parcial (desde hora X hasta fin del día)
-- Caso: is_available=false, start_time IS NOT NULL, end_time IS NULL
-- Interpreta como: bloqueado desde start_time hasta el final de la jornada
...
begin
  ...
  -- Fase 1a: Verificar si hay un override de día completo bloqueado
  if exists (
  ...
  -- Fase 1b: Verificar bloqueo parcial (desde hora X hasta fin del día)
  ...
  loop
    ...
    while v_cursor_time + (v_duration || ' minutes')::interval <= v_override.end_time loop
      -- Saltar si cae en bloqueo parcial
